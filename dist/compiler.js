...
function set(obj, key, val) {
  ...
  if (obj._isVue) {
    process.env.NODE_ENV !== 'production' && warn$1('Do not add reactive properties to a Vue instance at runtime - ' + 'delcare it upfront in the data option.');
    return;
  }
  var ob = obj.__ob__;
  if (!ob) {
    ...
    if (!vm.$options.render) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        if (vm.$options.template) {
          warn$1('You are using the runtime-only build of Vue where the template ' + 'option is not available. Either pre-compile the templates into ' + 'render functions, or use the compiler-included build.', vm);
        } else {
...
function renderElement(tag, data, namespace) {
  ...
  if (!parent) {
    process.env.NODE_ENV !== 'production' && warn$1('createElement cannot be called outside of component ' + 'render functions.');
    return;
  }
  if (!tag) {
...
if (process.env.NODE_ENV !== 'production') {
  ...
  strats.name = function (parent, child, vm) {
    if (vm) {
      warn$1('options "name" can only be used as a component definition option, ' + 'not during instance creation.');
    }
    return defaultStrat(parent, child);
  };
}
...
function assertProp(prop, name, value, vm, absent) {
  ...
  if (!valid) {
    warn$1('Invalid prop: type check failed for prop "' + name + '".' + ' Expected ' + expectedTypes.join(', ') + ', got ' + Object.prototype.toString.call(value).slice(8, -1) + '.', vm);
    return;
  }
