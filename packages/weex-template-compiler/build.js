...
function transformNode$3(el, options) {
    ...
    var _a = parseStaticClass(staticClass, options), dynamic = _a.dynamic, classResult = _a.classResult;
    if (process.env.NODE_ENV !== 'production' && dynamic && staticClass) {
        warn("class=\"" + staticClass + "\": " +
            'Interpolation inside attributes has been deprecated. ' +
            'Use v-bind or the colon shorthand instead.', el.rawAttrsMap['class']);
    }
...
function transformNode$2(el, options) {
    ...
    var _a = parseStaticStyle(staticStyle, options), dynamic = _a.dynamic, styleResult = _a.styleResult;
    if (process.env.NODE_ENV !== 'production' && dynamic) {
        warn("style=\"" + String(staticStyle) + "\": " +
            'Interpolation inside attributes has been deprecated. ' +
            'Use v-bind or the colon shorthand instead.', el.rawAttrsMap['style']);
    }
...
function preTransformNode$1(el) {
    if (isUnitaryTag(el.tag) &&
        !el.attrsList.some(function (item) { return item.name === 'append'; })) {
...
function nodeToBinding(node) {
    switch (node.type) {
        ...
        case "ArrayExpression":
            return node.elements.map(function (_) { return nodeToBinding(_); });
...
function generateBinding(exp) {
    if (exp && typeof exp === "string") {
        var ast = null;
        try {
            ast = acorn__default['default'].parse("(" + exp + ")", { ecmaVersion: 5 });
        }
        catch (e) {
...
function preTransformVBind(el) {
    for (var attr in el.attrsMap) {
        if (bindRE.test(attr)) {
            var name_1 = parseAttrName(attr);
            var value = generateBinding(getAndRemoveAttr(el, attr));
            delete el.attrsMap[attr];
            addRawAttr(el, name_1, value);
        }
    }
}
    ...
    if (hasConditionDirective(el)) {
        ...
        if (ifExp) {
            ...
            if (prevConditions.length) {
                ...
                options.warn("v-" + (elseifExp ? 'else-if="' + elseifExp + '"' : 'else') + " " +
...
function genDefaultModel(el, value, modifiers) {
    var _a = modifiers || {}, lazy = _a.lazy, trim = _a.trim, number = _a.number;
...
makeMap('richtext,transition,transition-group', true);
...
function compile(template, options) {
    ...
    if (options && generateAltRender) {
        ...
        var render = compiler.compile(template, options).render;
