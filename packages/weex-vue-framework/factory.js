...
function renderRecyclableComponentTemplate(vnode) {
    ...
    if (render) {
        ...
        warn("@render function not defined on component used in <recycle-list>. " +
            "Make sure to declare `recyclable=\"true\"` on the component's template.", vm);
    }
}
            ...
            if (isPromise(res_1)) {
                ...
                if (isDef(res_1.timeout)) {
                    ...
                    timerTimeout_1 = setTimeout(function () {
                        timerTimeout_1 = null;
                        if (isUndef(factory.resolved)) {
                            reject_1(process.env.NODE_ENV !== 'production'
                                ? "timeout (" + res_1.timeout + "ms)"
                                : null);
                        }
                    }, res_1.timeout);
                }
...
function pruneCache(keepAliveInstance, filter) {
    ...
    for (var key in cache) {
        ...
        if (cachedNode) {
            var name_1 = getComponentName(cachedNode.componentOptions);
...
makeMap('html,body,base,head,link,meta,style,title,' +
...
makeMap('svg,animate,circle,clippath,cursor,defs,desc,ellipse,filter,font-face,' +
...
function createStyle(oldVnode, vnode) {
    ...
    var batchedStyles = {};
    for (var name_1 in staticStyle) {
        if (staticStyle[name_1]) {
            supportBatchUpdate
                ? (batchedStyles[normalize(name_1)] = staticStyle[name_1])
                : elm.setStyle(normalize(name_1), staticStyle[name_1]);
        }
    }
    if (supportBatchUpdate) {
...
function enter(_, vnode) {
    ...
    var enterClass = data.enterClass, enterToClass = data.enterToClass, enterActiveClass = data.enterActiveClass, appearClass = data.appearClass, appearToClass = data.appearToClass, appearActiveClass = data.appearActiveClass, beforeEnter = data.beforeEnter, enter = data.enter, afterEnter = data.afterEnter, enterCancelled = data.enterCancelled, beforeAppear = data.beforeAppear, appear = data.appear, afterAppear = data.afterAppear, appearCancelled = data.appearCancelled;
...
function leave(vnode, rm) {
    ...
    var leaveClass = data.leaveClass, leaveToClass = data.leaveToClass, leaveActiveClass = data.leaveActiveClass, beforeLeave = data.beforeLeave, leave = data.leave, afterLeave = data.afterLeave, leaveCancelled = data.leaveCancelled, delayLeave = data.delayLeave;
    ...
    var transitionProperties = (stylesheet['@TRANSITION'] &&
    ...
    if (startState) {
        for (var key in startState) {
            ...
            if (process.env.NODE_ENV !== 'production' &&
                ...
                warn("transition property \"" + key + "\" is declared in enter starting class (." + startClass + "), " +
...
function parseStyle(vnode) {
    ...
    var _a = vnode.data, staticStyle = _a.staticStyle, staticClass = _a.staticClass;
    if (vnode.data.style || vnode.data.class || staticStyle || staticClass) {
        var styles_1 = Object.assign({}, staticStyle, vnode.data.style);
        // @ts-expect-error
        var cssMap_1 = vnode.context.$options.style || {};
        var classList = [].concat(staticClass, vnode.data.class);
        classList.forEach(function (name) {
            if (name && cssMap_1[name]) {
                Object.assign(styles_1, cssMap_1[name]);
            }
        });
        return styles_1;
    }
...
var TransitionGroup = {
        ...
        for (var i = 0; i < rawChildren.length; i++) {
            ...
            if (c.tag) {
                if (c.key != null && String(c.key).indexOf('__vlist') !== 0) {
                    ...
                    warn("<transition-group> children must be keyed: <" + name_1 + ">");
                }
            }
        }
        if (prevChildren) {
            ...
            this.kept = h(tag, null, kept_1);
            this.removed = removed_1;
        }
    ...
    methods: {
        getMoveData: function (context, moveClass) {
...
var isReservedTag = makeMap('template,script,style,element,content,slot,link,meta,svg,view,' +
...
makeMap('web,spinner,switch,video,textarea,canvas,' + 'indicator,marquee,countdown', true);
var isRuntimeComponent = makeMap('richtext,transition,transition-group', true);
makeMap('embed,img,image,input,link,meta', true);
