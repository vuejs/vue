...
- `modules`
  ...
  An array of compiler modules. For details on compiler modules, refer to its [type definition (in the flowtype, as ModuleOptions)](https://github.com/vuejs/vue/blob/dev/flow/compiler.js) and the [built-in modules](https://github.com/vuejs/vue/tree/dev/src/platforms/web/compiler/modules).
...
Parse a SFC (single-file component, or `*.vue` file) into a [descriptor (in the flowtype, defined as SFCDescriptor)](https://github.com/vuejs/vue/blob/dev/flow/compiler.js). This is used in SFC build tools like `vue-loader` and `vueify`.
