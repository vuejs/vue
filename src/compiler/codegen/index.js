  ...
  for (i = 0, l = dirs.length; i < l; i++) {
    ...
    if (needRuntime) {
      ...
      res += `{name:"${dir.name}",rawName:"${dir.rawName}"${dir.value
        ...
        : ''}${dir.arg ? `,arg:"${dir.arg}"` : ''}${dir.modifiers
...
function genScopedSlot(
  ...
  return (
    `{key:${key},fn:function(${String(el.slotScope)}){` +
...
export function genChildren(
  ...
  if (children.length) {
    ...
    return `[${children.map(c => gen(c, state)).join(',')}]${normalizationType
      ? `,${normalizationType}`
      : ''}`
  }
}
...
function genComponent(
  ...
  const children = el.inlineTemplate ? null : genChildren(el, state, true)
  return `_c(${componentName},${genData(el, state)}${children
    ? `,${children}`
    : ''})`
}
